 ...
 for each (var hilite in hilites) {
  ...
  if (bni) {
   ...
   if (startContainer && endContainer) {
    ...
    window.serialno = +hilite.hiliteSpans[0].hiliteSpanId-1; // ?
    hiliteRange(rangeToHilite);
   }
  }
 }
}
...
function hilitePartialNode(node, startOffset, endOffset) {
...
function boundaryNodeIndices(xString, xElement) {
 ...
 var nodeTexts = nodes.map(function (node) {return node.textContent});
...
function hiliteRange(range) {
 ...
 if (sni == eni) {
  spans.push(hilitePartialNode(nodes[sni], sci, eci));
 }
 else {
  spans.push(hilitePartialNode(nodes[sni], sci, null));
  for (var k = sni+1; k <= eni-1; k++) {
   spans.push(hilitePartialNode(nodes[k], null, null));
  }
  spans.push(hilitePartialNode(nodes[eni], null, eci));
 }
 return spans;
}
