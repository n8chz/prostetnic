var FindScript = (function () {
  ...
  if (searchInput = document.getElementById("search")) {
   ...
   // console.log("searchInput *should* be focused");
   searchInput.addEventListener("input", function (event) { // ?
     // console.log("change event on #search");
     ...
     if (wordArray.length > 0) {
      // console.log("at least one word in wordArray");
      var data = document.getElementById("data");
      // console.log("#data element"+data.outerHTML);
      // h/t http://stackoverflow.com/a/683460/948073
      while(data.firstChild) {
       data.removeChild(data.firstChild);
       // console.log("one row removed");
      }
      // console.log("about to emit \"searchTextChange\"");
  ...
  self.port.on("newRow", function (newRowJSON) {
    // console.log("\"newRow\" received: "+newRowJSON);
