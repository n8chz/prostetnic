...
function matchingPages(wordsJson) {
    ...
    while (statement.executeStep()) {
        ...
        for (k = 0; k < 4; k++) {
            ...
            if (k==1) url=statement.getString(k);
        }
        // new code to incorporate page titles
        var titleStmt = main.placesConn.createStatement("select title from moz_places where url = :url");
        titleStmt.params.url = url;
        if (titleStmt.executeStep()) {
            newRow.push(titleStmt.getString(0));
        } else
        newRow.push(null);
        // end new code
