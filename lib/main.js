  ...
  Command.prototype = {
   ...
   makeButton: function () {
    return this.api.actionButton({
      id: this.options.image.split("/").pop().split(".")[0], // kludge
...
*/
   ...
   this.highlight = new Command(this.api, {
     image: this.api.data.url("hilite.svg"),
   ...
   this.svgWorker.port.on("newIcon", function (b64code) {
     system.highlight.button.icon = "data:image/svg+xml;base64,"+b64code;
...
/*
   this.highlight = new Command("Highlight current selection (Ctrl-Alt-H)", "./hilite.svg", function () {
     system.storage.hiliteSpanID = system.getHiliteID();
     system.page.port.emit("hilite", JSON.stringify(system.storage));
   });
   this.undo = new Command("Undo most recent highlight (Ctrl-Alt-Z)", "./undo.svg", function () {});
   this.erase = new Command("Erase all highlights on this page (Ctrl-Alt-N)", "./erase.svg", function () {});
   this.find = new Command("Search for words in previously highlighted pages (Ctrl-Alt-F)", "./search.svg", function () {});
*/
  ...
  HighlighterSystem.prototype = {
   ...
   setupDB: function () {
    ...
    this.generateDB();
//    // While we've got the Components variables in-scope, let's get the URL parser on board:
//    this.eTLDService = Components.classes["@mozilla.org/network/effective-tld-service;1"].
//                  getService(Components.interfaces.nsIEffectiveTLDService);
//    // see https://developer.mozilla.org/en-US/Add-ons/Code_snippets/URI_parsing
   },
...
/*
   getMatchingPages: function (words) {
    var queryString = "select * from instances where";
    queryString += (new Array(words.length + 1)).join(" text like \"%\"||?||\"%\" ").split("  ").join(" and ");
    var statement = this.dbConn.createStatement(queryString);
    var urlList = [], url, records = [];
    while (statement.executeStep()) {
     url = statement.getString(1);
     if (records.length <= 50 && urlList.indexOf(url) != -1) {
      urlList.push(url);
      records.push({
        date: statement.getString(0),
        textContent: statement.getString(2)
      });
     }
    }
    return records;
